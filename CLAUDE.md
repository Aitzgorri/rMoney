...
- **Button tooltips (MANDATORY)**: Every button MUST have a tooltip (`title` attribute) describing what it does — **no exceptions**. This matters most for icon-only buttons (⇄, ⚙, ✎, ×, ↓/↑, chevrons, drag handles), but text buttons get one too when the label alone doesn't convey the effect. When adding any new button, add the `title` in the same change. Style: concise imperative ("Delete this envelope (asks for confirmation)"); toggles use a state-aware ternary. Where a **custom CSS tooltip** already exists (`data-tooltip` in Transactions, `data-tip` in Settings), that satisfies the rule — use it **instead of** `title`, never both (two tooltips would render at once). (Rule added 2026-07-08; the one-time app-wide audit shipped in Phases 54d/54e — every existing button is covered, so any button without a tooltip is a regression.)
...
- **Public releases: scrap Device-Sync data (MANDATORY, rule added 2026-07-10).** Whenever preparing a **public version of the app** — release installers, a GitHub release, demo datasets, screenshots, or ANY backup file that leaves the user's own machines — it must contain **no Device-Sync data**: the WebDAV folder URL and username (in the `rmoney_settings` blob), the sync password, device ids / sync state (`rmoney_sync_meta`, `rmoney_sync_base`), and the sync deletion log. Installers contain no user data by construction; this rule guards every *other* published artifact. ✅ **Gap closed (Phase 67, 2026-08-21):** the Sharable (redacted) export now scrubs the Device-Sync footprint itself — `redactForFileExport` in `data/portability.js` drops `settings.sync` (WebDAV URL + username) and the `deletions` tombstone log on top of the credential redaction, so a Sharable backup is safe to share without manual scrubbing. The **sync payload keeps both** (it still calls `redactExportData`) because propagation depends on them — never fold the strip back into `redactExportData`, it would break deletion propagation silently. Release-checklist enforcement: RELEASE.md → Step 5b. ⚠ Backups made **before** Phase 67 still contain the URL + username — scrub those by hand.
