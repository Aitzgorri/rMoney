...
- [x] Pre-migration backups (`.rmy` files exported under v0.32.0 or earlier) load cleanly into v0.34.0: the import path runs the same migration over the loaded data before commit (`migrateBackup` calls the pure `migrateDividendsArrayToV2` helper before `importAppData` writes to localStorage), so the loaded records pick up `status: 'received'` automatically. See SPEC-016 § Backup format versioning + migration for the broader v2 backup discussion. Verified on the real app 2026-05-28 before tagging v0.34.0.
...
- [x] **Share count auto-recalculates** for pending-payment records when the user records a buy/sell that changes the shares held on `exDividendDate − 1`. Specifically: any pending-payment record whose `exDividendDate > today` re-derives `shareCount` from `getOpenLots(investingAccountId, ticker, asOfDate = exDividendDate − 1)` on every read. Once `exDividendDate ≤ today`, the share count is locked in (the user is entitled to the dividend on however many shares they held on the record date, even if they sold them later). Implemented in 33f via `promoteDividends()` final-pass recalc — see plan item 417.
