...
### Sharable-export Device-Sync scrub *(Phase 67 ✓ 2026-08-21 — resolves the 2026-07-10 open question)*
- [x] The sync payload and the human-shared file export no longer share one redaction. `data/portability.js` keeps **`redactExportData`** (credentials only — used by `utils/sync.js` `buildLocalSnapshot`) and adds **`redactForFileExport`** = `redactExportData` **plus** `delete out.deletions` and `delete out.settings.sync`. `App.jsx` `writeBackupFile` is the only caller of the latter.
- [x] A **Sharable export** therefore contains no Device-Sync footprint: no WebDAV folder URL, no sync username, no `webdavPasswordSet`/`enabled` flags, and no `deletions` tombstone log. (The sync **password** was never in any payload — it lives in the secrets backend under `sync/webdav/password`.)
- [x] The **sync payload is unchanged** and must stay that way: `settings.sync` is how a second device learns the folder config, and `mergeSnapshots` reads `deletions` from *both* sides to keep deleted records from resurrecting. Folding the scrub into `redactExportData` would break deletion propagation silently — no UI surfaces it — so unit tests assert the sync path **keeps** both fields, from the opposite direction to the export tests.
- [x] A **Full backup is unaffected** and keeps both fields. This makes the two modes mean distinct things: *Full = personal restore, everything; Sharable = safe to hand to someone else.* **Consequence to be aware of:** restoring from a Sharable backup and then enabling sync starts with no tombstones, so records deleted on another device can reappear on the first merge. Restore from a **Full** backup whenever the goal is to rebuild your own device.
- [x] **No backup-format bump.** Both modes still write `rmoney-data-v7`; the Sharable payload is a subset, and `importAppData` already defaults `deletions ?? []` while `getSyncConfig()` falls back to `{ url: '', username: '', webdavPasswordSet: false, enabled: false }` when `settings.sync` is absent. A scrubbed export loads into any v7-capable build with Device Sync simply switched off.
- [x] Unit tests in `data/portability.test.js` (9 cases): export drops `settings.sync` + `deletions` and the values survive nowhere in the serialised payload; credentials still redacted; non-sync data intact; caller payload not mutated; sync path keeps both fields; a scrubbed export imports to the unconfigured sync shape.
...
## Open Questions
- ~~**The Sharable (redacted) export keeps the WebDAV URL + username**~~ (raised 2026-07-10) — **RESOLVED in Phase 67 (2026-08-21)**, by the code fix this entry proposed. The two consumers of the redaction now differ by exactly one function: `redactExportData` (sync payload, keeps the sync fields) and `redactForFileExport` (file export, drops them). See *Sharable-export Device-Sync scrub* under Acceptance Criteria. The procedural mitigation in RELEASE.md Step 5b is no longer the only line of defence.
