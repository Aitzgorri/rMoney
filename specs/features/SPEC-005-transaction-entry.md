...
- [x] **Transfer From-account prepopulation** *(Phase 54b)*: the transfer branch's From account follows the same filter → last-used rule as the expense account (`defaultAccountId`), and the To account defaults to the first *different* account so the form never opens with source = destination
