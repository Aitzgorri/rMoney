...
- [ ] **Every currency input in the app is a dropdown** (not a free-text field). Applies to: Account form (SPEC-002), Buy form, Sell form, Dividend form, Add manual stock dialog, Resolution dialog "manual entry" row, Cash balance form / cross-currency deposit / standalone exchange (SPEC-018), Planning income / expense rows (SPEC-009), Budget form (SPEC-011), Trading fees Settings rows (SPEC-034), Trading scenarios Currency display picker (SPEC-034), Stock profile Edit dialog (SPEC-033). No exceptions.
- [ ] **Favorites at the top, others below — visually separated.** The dropdown renders the favorites list first in user-defined order (bold), followed by a thin divider line (`────`), followed by every other available currency sorted alphabetically ascending. Selected option is highlighted; the divider is a non-selectable disabled `<option>` so keyboard navigation works correctly.
- [ ] **Favorites managed in Settings → General, directly below the main-currency setting.** The card title: "Favorite currencies". A drag-handle list of the user's favorites (one row per currency: ISO code · full name · remove button). An "Add currency" input below the list opens a searchable picker over every supported ISO 4217 code. Reorder via drag-and-drop (`@dnd-kit/core` already in deps).
- [ ] **Defaults on first run.** The favorites list pre-populates with the existing `SUPPORTED_CURRENCIES` constant (currently 14 ISO codes: AUD, CAD, CHF, CNY, CZK, DKK, EUR, GBP, HUF, JPY, NOK, PLN, SEK, USD) in that order. Migration is one-shot at app boot when `settings.favoriteCurrencies` is undefined.
- [ ] **Adding a new currency to favorites:** picker opens with every ISO 4217 currency available (full list sourced from a static module `utils/iso4217.js` to avoid runtime dependencies). Currencies already in favorites are greyed out. Selecting adds to the bottom of the favorites list (user can drag to reorder).
- [ ] **Removing a favorite:** the row's × button removes it from favorites; it remains available in the "others" section of every dropdown (alphabetical). Cannot remove the main currency from favorites (the × button is disabled with a tooltip).
- [ ] **Main currency is always in favorites.** When the user changes main currency, if the new main isn't in favorites yet it is auto-added at the top of the list.
- [ ] **Storage.** `settings.favoriteCurrencies: string[]` (array of ISO codes in user-defined order) added to the existing `rmoney_settings` blob; no separate localStorage key, no new Storage tab card required.
- [ ] **Shared component.** `components/CurrencyDropdown.jsx` — props: `value`, `onChange`, `disabled?`, `excludeMinorUnits?` (when true, suppresses minor-unit codes like `GBp` that should never be user-selected). Internal: reads `settings.favoriteCurrencies` and `utils/iso4217.js`, renders the favorites + divider + others. Used by every form listed above; the existing inline `CURRENCIES = ['EUR', 'USD', ...]` constants in AccountForm, Planning, etc. are deleted in favor of this component.
...
- `settings.favoriteCurrencies` (string[], ISO 4217 codes in user-defined order) — Phase 33; defaults to existing `SUPPORTED_CURRENCIES`
