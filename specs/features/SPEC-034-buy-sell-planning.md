...
- [x] **Currency-exchange priority** — implemented per spec in `simulateCashImpact()`. *(Sub-phase 32h item 380)* *(Phase 38 refines this: sells now repay any buy-created FX leg in their currency before crediting the trade-currency balance — see the Phase 38 section.)*
...
- [x] **Cash-impact and dividend-impact tables — column alignment fix.** Header cells render with the same `text-align` as their column's value cells. Currency code prefix lives in its own narrow column (right-aligned text) so numeric values line up by decimal across all rows. Start column always shows a value (or "0.00" when literal zero, never blank). Applied consistently to the cash-impact table (Start / Top-up / Sells / Buys / Transfer in / Transfer out / End / Overspend) and the dividend-impact table (Sells avg yield / amount / Buys avg yield / amount / Δ rows). *(Sub-phase 33l)* **⚠ Implementation bug — see Phase 38 item below: the shipped CSS does not actually right-align the headers.**

### Phase 38 — June 2026 adjustments

- [ ] **Cash-impact / dividend-impact header alignment — CSS specificity bug fix.** The Phase 33l criterion above is correct in *intent* but the shipped CSS does not achieve it. In [`BuySellPlanning.module.css`](../../app/src/screens/BuySellPlanning.module.css), the rule `.impactTable th { text-align: left }` (selector specificity 0,1,1) **overrides** the `.tdRight` class (specificity 0,1,0) applied to each numeric header cell, so the headers (Start / Top up / Sells / Buys / Transfer in / Transfer out / End / Overspend, plus the dividend-impact yield/amount headers) render **left-aligned** while their values render right-aligned — the header label does not sit visually above the column it names. Fix: make the right-align win for header cells — either `.impactTable th.tdRight { text-align: right }` (raise specificity), or drop the blanket `text-align: left` on `.impactTable th` and apply left-align only to the label / empty currency-code header cell. The currency-code column header (`.ccyCol`, already `text-align: right`) is unchanged. No data or layout change beyond the header `text-align`.
- [ ] **Sells unwind buy-driven FX legs before crediting their own trade currency.** Today `simulateCashImpact()` ([`planningCalc.js`](../../app/src/utils/planningCalc.js)) applies every included sell purely as a credit to its own trade-currency balance, then runs the buy cascade (same trade currency first, then an FX leg from main / other currencies on a shortfall). That can show a buy FX-borrowing from currency **B** while a sell simultaneously piles cash into currency **A**, even when the two should net out. **New rule:** a sell whose proceeds are denominated in a currency that an included buy had to *borrow into via an FX leg* first **repays that leg** — the sell proceeds reduce the `transferIn` / `transferOut` pair the buy created (restoring the tapped currency and shrinking the simulated FX movement), and only the remainder lands as net cash in the sell's own trade currency. The cascade therefore minimises the **net** FX legs across the whole scenario instead of treating sells and buys independently. Worked example: GBP cash £0, EUR cash €1000; a buy needs £500 → FX-borrows ~€590 from EUR (EUR €1000 → ~€410, GBP +£500 spent). Add a sell yielding £400 → instead of leaving GBP at +£400, the £400 repays ~€470 of the borrow (EUR back to ~€880) and GBP nets £0; only proceeds **beyond** the borrowed amount remain in GBP. Order of operations in the rewritten `simulateCashImpact()`: (1) seed balances + top-ups; (2) compute each included buy's gross demand and the FX leg(s) it needs (trade currency first, then main, then others by descending value — unchanged priority); (3) apply sells, each first repaying any outstanding buy FX leg **in the sell's currency** (largest borrowed leg first), remainder credited to the trade-currency balance; (4) finalise `end` and `shortfall` per currency. The **Overspend** column still reports demand the cascade cannot satisfy after netting. **On the note's "decreasing number of stocks" point:** because the simulation is recomputed from scratch on every edit and always fills trade-currency cash to its maximum before borrowing, lowering a buy's share count *already* shrinks the borrowed-from-other-currency leg first — that reversibility is a property of the from-scratch recompute, documented here so it isn't accidentally "optimised" into stateful incremental bookkeeping later.
