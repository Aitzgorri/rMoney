...
- [x] **Editable FX rates panel** — every distinct trade-currency → main-currency pair shows the live SPEC-027 rate in a read-only **Live** column plus an editable **Override** column; overrides stored in `scenario.fxOverrides`. *(Sub-phase 32h item 379b)*
  - The **Live** column always shows the genuine live spot rate (or `—` when the cache has not supplied that pair), computed from a pure-live FX map that **never** includes the override — so the Override field reads as a true override against a visible baseline. *(2026-06-02 fix)*
  - Clearing the Override field deletes the key from `fxOverrides`; the effective rate then falls back to the live spot automatically (the effective map is seeded from the live map and only replaces a pair when the override parses as a number `> 0`). An empty or zero override is ignored — it never leaves an empty/stale rate behind that would break the cash-impact cascade. *(2026-06-02 fix — previously the panel's "Live" column read the merged effective value, so an override masqueraded as the live rate and clearing it left the cell blank.)*
...
- [x] **Cash-impact / dividend-impact tables scroll horizontally on narrow viewports.** Both wide Overview tables are wrapped in a `.tableScroll` (`overflow-x: auto`) container — mirroring the ConfigurableTable `.tableWrap` pattern used by the Sells/Buys row tables — so on a narrow (mobile) viewport the table content scrolls horizontally instead of overflowing its card. No change to the tables' columns or content. *(Phase 38)*
