...
- [x] The **Freq and Day columns render human-readable labels** via the shared `FREQUENCY_LABELS` / `dayLabel` helpers ("Bi-weekly", "Tuesday", "16th") — never the raw stored values (`biweekly`, weekday integer `2`) — matching the envelope detail pane's scheduled list; rows without a day render "—" *(Phase 53c)*
- [x] Scheduled (regular) transfers support the shared frequency set (Phase 47): **weekly, bi-weekly, monthly, quarterly, yearly** — created via the SPEC-004 transfer form, which reads its options from `utils/frequency.js`. The execution engine (`runDueScheduledTransfers`) fires each frequency on the correct day: weekday for weekly/bi-weekly, day-of-month for monthly/quarterly/yearly. **Bi-weekly, quarterly and yearly anchor on the rule's optional `startDate`, falling back to `createdAt`** *(Phase 53f — previously createdAt only)*: bi-weekly fires every 14 days from the first matching weekday on/after the anchor; quarterly/yearly fire on the day-of-month every 3rd / 12th month from the anchor month. An explicit start date also **gates every frequency** — nothing fires before it (enables "starts next month" rules). The due-date check uses the **local** calendar date (no UTC `toISOString` shift)
