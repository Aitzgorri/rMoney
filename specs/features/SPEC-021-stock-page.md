...
- [ ] **Single-line per row with short headers + tooltips (Phase 32 / item 366):** The unified dividend list renders **all data for a payout on one visual line** — no subline, no wrap. Columns: `Ex-div | Pay | Per share | Shares | Tax % | Net | Type | Source | Account | (actions)`. Headers use compact short labels; each header carries a tooltip with the full meaning (`Per share` → "Gross dividend per share, before tax, in the stock's trading currency"; `Type` → "Regular or Special dividend"; etc.). The `Special` indicator becomes a column value (a chip rendered inside the `Type` column) instead of an inline element next to `Per share` — so it does not push the row to a new line. When trading currency differs from main currency and the currency-toggle is set to Main, an extra "Net (main)" column becomes visible. The user controls visibility/order of all non-essential columns via the standard `ConfigurableTable` column-picker; `Ex-div`, `Pay`, `Per share`, `Net`, and `(actions)` are always-visible essentials. Reason: even with sublines, current rendering takes 2–3 lines per row when the dividend is special or the account name is long; a single-line layout with tooltips on short headers fits more rows per screen and is easier to scan.
