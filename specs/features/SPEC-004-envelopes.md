...
- [x] **Favorite envelopes in the transfer form** *(Phase 53e)*: both the From and To selects show a **Favorites** optgroup (★, user order, stale ids skipped) above the full indented tree, via the shared `components/optionHelpers.jsx` — the same convention as the transaction form's envelope dropdown (SPEC-005)
