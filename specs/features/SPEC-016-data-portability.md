...
- [x] **Sharable export also drops the Device-Sync footprint** (SPEC-039, Phase 67): `settings.sync` (WebDAV folder URL, username, `webdavPasswordSet`, `enabled`) and the `deletions` tombstone log. Implemented as `redactForFileExport` in `data/portability.js` — `redactExportData` **plus** those two deletions. The distinction matters: the device-sync payload reuses `redactExportData` and *depends* on both fields (`settings.sync` propagates the folder config; `mergeSnapshots` needs `deletions` on both sides so deleted records cannot resurrect), so the strip lives on the file-export path only. No format bump — the Sharable payload is a subset of `rmoney-data-v7` and the loader already defaults both keys.
- [x] The two modes therefore mean distinct things: **Full backup = rebuild your own device** (everything, incl. sync config and tombstones, plus the embedded Stronghold vault on Tauri); **Sharable export = safe to hand to someone else** (no credentials, no NAS address, no deletion history). ⚠ Restoring from a *Sharable* backup and then enabling sync starts with no tombstones, so records deleted on another device can reappear on the first merge — use a **Full** backup to rebuild your own machine.
