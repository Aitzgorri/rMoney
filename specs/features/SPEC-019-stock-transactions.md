...
- [x] **Stock profile resolution (SPEC-029):** when the user enters a ticker that has no resolved `name` on its `stockProfile`, a **"Look up"** button appears that runs the SPEC-029 resolution dialog. The user confirms a candidate (or enters manually); the resolved `name`, `stockExchange`, and `currency` are persisted to the profile. If the buy form's `stockExchange` or `currency` fields were empty, they are pre-filled from the resolved profile. **Resolution is offered but is not a save gate** — a buy can be saved with an unresolved ticker, so a CSV import that can't auto-match a symbol, or an offline session with no provider reachable, never blocks recording the trade. Unresolved stocks surface in the Stock inventory page's **Unconfirmed** filter (SPEC-033) — which CSV import deep-links to — so the user can confirm them later.
...
- [x] **Cross-currency source on buy (item 159b):** the buy form shows a "Pay from" dropdown listing all account cash balances. When the selected balance's currency differs from the trade's currency, a "Currency exchange required" section appears with rate, computed source amount, and optional FX fee. Saving the buy first creates the buy record (which auto-creates/uses the matching-currency cash balance), then creates a companion `currency-exchange` stock-transaction record with `triggeredByStockTransactionId` set to the buy's ID. The negative-balance check is performed on the source balance (not the intermediate trade-currency balance). Deleting the buy also cascades to delete the triggered exchange.
...
- [x] **Proceeds destination cash balance selector (item 164):** the sell form shows a "Proceeds into" dropdown. Default is the matching-currency balance. When a different-currency balance is selected, a "Currency exchange triggered" section appears with rate and optional FX fee. After saving the sell, a standalone `currency-exchange` record converts the net proceeds from the trade-currency balance to the selected destination balance.
...
### Splits
- [x] **API-detected split notifications (Phase 36d).** On stock-page mount, `getCorporateActions(ticker, fromDate)` is called via the SPEC-027 chain (Finnhub `/stock/split`, Twelve Data `/splits`, etc.) with a 5-year lookback. Splits already present as user `type: 'split'` transactions (matched by date + numerator + denominator) and splits the user has explicitly dismissed are filtered out. Whatever remains is rendered as one amber banner per pending split below the page header: "Detected {n}-for-{d} split on {TICKER} effective {YYYY-MM-DD} — apply?" with Apply / Dismiss buttons. The provider chain failing silently returns no banners.
...
- [x] On **Dismiss**: the banner disappears for that session and the `(ticker, date, ratio)` triple is persisted to `rmoney_dismissed_splits` localStorage so it isn't re-surfaced on future visits. Lots are not modified; the split can still be entered manually. The Settings → Storage tab exposes a "Dismissed split notifications" card with a Clear button to forget all dismissals.
...
### Editing & deleting transactions (Asset-movements ledger + stock page)
Both the account-detail **Asset movements** ledger (SPEC-018) and the **stock page** transaction list (SPEC-021) let the user edit or delete the records they show. Each row **expands** to a detail panel whose footer carries an **Edit** button and a red **Delete** button side by side (the delete previously sat as a `×` on the collapsed ledger row — it now lives in the expanded detail to match the crypto-swap layout). Delete is routed through `canDeleteStockTransaction` (guard) → confirmation dialog → `deleteStockTransaction` (cascade). Buy/sell edit uses the **shared `BuyEditForm` / `SellEditForm`** (extracted to `components/StockTxEditForms.jsx`) plus the shared `applyBuyEdit` / `applySellEdit` helper (`data/stockTxEdit.js`), so editing behaves identically from either screen — the cost-basis cascade lives entirely in `updateBuy` / `updateSell`. Deposits/withdrawals, dividends, currency exchanges, and crypto swaps/wallet-transfers were already deletable; this extends deletion to **buy, sell, and transfer** so every asset transaction shown can be removed:
- [x] **Buy** rows are deletable, **blocked** (with a reason naming the blocking records) when any sell/transfer/swap has drawn from the buy's lot (item 93). On delete, the buy's `buy` / `buy-fee` cash movements and any buy-triggered `currency-exchange` (plus its cash movements) are removed; positions, open lots, weighted-average cost, and realized P/L all recompute at read time.
- [x] **Sell** rows are deletable (never blocked — nothing draws from a sell). The `sell` / `sell-fee` cash movements are removed and the lots the sell had consumed become available again; realized P/L recomputes. A **standalone sell-proceeds currency exchange** (item 164) is not linked back to the sell, so it is not auto-removed — it stays independently deletable from its own ledger row.
- [x] **Transfer** rows (the `transfer-fee` line) are deletable, **blocked** when sells in the destination account consumed the transferred-in lots; on delete the `transfer-fee` cash movement is removed and the destination's synthetic lots disappear on recompute.
- [x] Each deletion is confirmed in a dialog naming the record type; a blocked deletion shows the explanatory reason instead of a confirm button.
- [x] **Out of scope (unchanged):** **splits** have no cash leg and do not appear in this ledger (their correction path is deleting the split record per the Splits §); a **fee-less transfer** produces no ledger row and so is not deletable from here.
