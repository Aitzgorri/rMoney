...
- [ ] **Action-modal z-index above fullscreen overlay (Phase 32 / item 369):** when the user clicks a row action (Sell / Dividend / row detail) on the Positions table while the table is in fullscreen, the action's modal must render **above** the fullscreen overlay so the user can interact with it. Currently the fullscreen overlay covers the modal. Fix by giving the BuyForm / SellForm / DividendForm overlay a z-index strictly above `ConfigurableTable.fullscreenOverlay`, or by attaching action-modals to a portal at the document body root rather than nesting them inside the fullscreen container. Same fix applies anywhere `ConfigurableTable` is used (Investments overview Positions, future Reports table tab in Phase 29d).
