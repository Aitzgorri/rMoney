...
- [x] The existing root file `Import_test.csv` is **moved** into a `fixtures/` directory, or **deleted** before first push if it contains real personal data. Moved to `fixtures/Import_test.csv`; confirmed it contains sample stock trade data, not personal bank data. **The security goal is met — the file was never tracked.**

  > **Correction (2026-08-21).** This criterion originally claimed `fixtures/` "is not ignored — small public-safe fixtures are fine". **That was never true.** The root `.gitignore` matches `*.csv` at any depth with no `!fixtures/` negation, so `fixtures/Import_test.csv` was ignored from the moment it was moved (`git check-ignore -v` → `.gitignore:2:*.csv`). It was therefore never committed, and was lost with the machine in the August 2026 rebuild — a fresh clone had nothing to restore. The claim was also internally inconsistent with `pre-publish-audit.sh`, which **fails the build if any `.csv` is tracked**: had `fixtures/` genuinely been un-ignored, committing the fixture would have tripped the audit.
  >
  > **Resolved by deciding the current behaviour is the desired one:** CSVs stay ignored everywhere, `fixtures/` included, and **no example import CSV is kept in the repo**. The app has no CSV writer — `screens/CsvImport.jsx` only reads (`FileReader`, `accept=".csv,.txt"`) — so import files come from the user's own broker export (SPEC-025) and a real one is always a re-download away. Nothing in the repo depends on a fixture: no test reads one. Adding a tracked CSV later would require a deliberate `!` negation here **and** a matching exception in the audit script; do not add one without changing both.
