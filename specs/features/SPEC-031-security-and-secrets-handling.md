...
- [x] `app/src-tauri/tauri.conf.json` `app.security.csp` is set to a strict static base whose `connect-src` enumerates every host the app may reach: `'self'`, the seven market-data providers (Yahoo Finance × 2 hostnames, Polygon/Massive, Twelve Data, Alpha Vantage, Finnhub, Stooq) and the two supported AI providers (`api.anthropic.com`, `api.openai.com`). `'unsafe-inline'` for styles is required by Vite's runtime style injection; everything else is locked down.
