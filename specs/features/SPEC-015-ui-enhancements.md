...
- [x] **Investing-side audit** *(Phase 54e, 2026-07-09)*: the remaining **31 files** swept (InvestingAccountDetail + its dialogs, StockPage, AiChatPanel, Benchmarks, DividendPage, InvestmentReports, Portfolios, Investments, StockInventory, BuySellPlanning, CsvImport, watchlists, Accounts + form, and the security/passphrase dialogs) — **~362 titles added**, plus **App.jsx's 8 banner/dialog buttons** (missed by both sweeps' file lists, caught by the closing app-wide scan). Security-dialog titles state their effects accurately per SPEC-031 (full backup embeds the encrypted vault; reset/replace warn about irreversibility). **Final state: 685 buttons app-wide, 0 without a tooltip.**
