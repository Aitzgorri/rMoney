...
54d. [ ] **Tooltip audit — budgeting side.** Sweep the budgeting screens/components (nav, Dashboard, Transactions, forms, Envelopes/History, Planning, Bills & Income, Scheduled transfers, Budgets, Payees, Settings-general): sized 2026-07-09 at ~137 buttons missing `title` out of ~171. Every button gets a meaningful `title` per the MANDATORY CLAUDE.md rule (D2). (SPEC-015)
54e. [ ] **Tooltip audit — investing side.** Same sweep for the investing screens (InvestingAccountDetail, StockPage, BuySellPlanning, DividendPage, InvestmentReports, watchlists, inventory, CSV import, Settings-investments…) — the app-wide total is ~685 buttons with ~450+ missing titles, the bulk here. (SPEC-015)
