...
| 61 — notes_9 small wins | planned | Transfer-button wrap fix, scheduled-sum header, Settings → Sync tab, account filter rows + favorites, transfer direction blues; SPEC-006/007/012/039 |
| 62 — Transactions list pagination | planned | 50-row incremental rendering + the pre-existing unchecked SPEC-006 type-filtered category dropdown; SPEC-006 |
| 63 — Period-summary drill-down | planned | Expandable Income/Expense rows, 10-row chunks, column picker via ConfigurableTable, click-to-edit; SPEC-008 |
| 64 — Scheduled-transfer occurrence overrides | planned | Port the Phase-55d override model (skip / one-time date-amount) to scheduled envelope transfers; SPEC-012/004/007 |
| 65 — Multiple envelope plans | planned | Named plans + one ACTIVE plan (P1), reusing the trading-scenarios CRUD pattern; backup → v7; SPEC-009 |
...
## Phase 61 — notes_9 small wins (planned)

61a. [ ] **"⇄ Transfer" button wraps to two rows on desktop (bug).** Root cause: `.headerActions` ([Envelopes.module.css:90](../app/src/screens/Envelopes.module.css)) has no `flex-wrap` and `.transferBtn` no `white-space: nowrap`, so the label breaks inside the pill when the header is crowded. Fix: `white-space: nowrap` (+ `flex-shrink: 0`) on the button. (SPEC-007)
61b. [ ] **Scheduled-transfers header sum (P3).** The envelope detail-pane "Scheduled transfers" header ([EnvelopeHistory.jsx:281](../app/src/screens/EnvelopeHistory.jsx)) gains "+in −out = net / month" as monthly equivalents next to the count badge. (SPEC-007/012)
61c. [ ] **Settings → Sync tab.** New `sync` tab in `Settings.jsx` `TABS`; the Device-sync card moves there from General (SPEC-039's original UI sketch already said "Settings → new Sync section"). Future sync surface (change/conflict log) lands there too. (SPEC-039)
61d. [ ] **Transactions account filter: one row per account, favorites first.** The desktop left-column account chips ([Transactions.jsx:288-306](../app/src/screens/Transactions.jsx)) become a vertical list, one account per row, ordered via the shared `splitFavorites` (favorites in user order, divider, rest) — matching the dropdown convention. Mobile keeps the wrapped chips. (SPEC-006)
61e. [ ] **Transfer direction colors (P4).** Single-account view: incoming transfers lighter blue, outgoing darker blue (replacing neutral); All-accounts view unchanged. (SPEC-006)

## Phase 62 — Transactions list pagination (planned)

62a. [ ] **Incremental rendering, 50 rows per chunk.** Render the first 50 filtered rows; an IntersectionObserver sentinel loads the next 50 on scroll-bottom (reuse the DividendPage pattern, [DividendPage.jsx:1054](../app/src/screens/DividendPage.jsx)). Filters, the summary strip, and the running balance still compute over the **full** filtered set — display-only pagination, no behaviour change. (SPEC-006)
62b. [ ] **Housekeeping — type-filtered category dropdown.** The one pre-existing unchecked SPEC-006 criterion (filter-panel category dropdown scoped by the active type filter, Income/Expense headers when untyped), which was missing from this plan. (SPEC-006)

## Phase 63 — Dashboard period-summary drill-down (planned)

63a. [ ] **Expandable Income / Expenses rows.** In each currency block of the Period Summary card ([Dashboard.jsx:293-325](../app/src/screens/Dashboard.jsx)), clicking Income or Expenses expands the underlying transactions in an in-card scrollable list, loading **10 rows per chunk** as the user scrolls to the bottom. (SPEC-008)
63b. [ ] **Columns + column picker.** Row columns: Date · Payee · Category · Envelope · Amount · Currency, with a select-columns control persisted per user — via the shared `ConfigurableTable` (same mechanism as Investment Reports). (SPEC-008)
63c. [ ] **Click-to-edit.** Clicking an expanded row opens that transaction for editing (SPEC-005 form), mirroring the Payees report. (SPEC-008)

## Phase 64 — Scheduled-transfer occurrence overrides (planned)

64a. [ ] **Override model on scheduled transfers.** Port the Phase-55d model to `data/envelopes.js`: `overrides: { [seriesDate]: { date?, amount?, skipped? } }` on the scheduled-transfer record (additive, no bump); `isScheduledTransferDueToday` / `runDueScheduledTransfers` / `nextScheduledOccurrence` become override-aware (skip suppresses one firing; date/amount overrides apply to that occurrence only, consumed one-shot; pull-earlier and push-later both work — mirror the bills.js engine semantics). Tests first per the rule. (SPEC-012/004)
64b. [ ] **Override UI on both surfaces.** Generalize `OccurrenceOverrideDialog` (currently hard-wired to bill/income item shape) or build a sibling for transfers; opened from rows on the Scheduled-transfers page **and** the envelope detail-pane scheduled list. Both lists show the override-aware effective next date/amount with the ↻ marker, matching the Dashboard/Bills pattern. (SPEC-012/007)

## Phase 65 — Multiple envelope plans (planned)

65a. [ ] **Plans data layer.** Named-plan registry + `planId` scoping for planned incomes, planned expenses and (Phase 66) allocation rows, reusing the trading-scenarios CRUD pattern ([tradingScenarios.js](../app/src/data/tradingScenarios.js): create/rename/duplicate/delete); a boot migration wraps existing data as the first plan and marks it active. Active-plan id is synced app data (P1), not per-device. (SPEC-009)
65b. [ ] **Plan switcher UI.** Visual plan boxes on the left side of the Planning page (per the notes — akin to the investing account tiles), with new/rename/duplicate/delete actions; switching selects which plan the page shows. (SPEC-009)
65c. [ ] **Active-plan semantics (P1).** Only the active plan shows sync indicators and offers Reset/Apply; drafts are freely editable with those controls absent/disabled; an explicit "Make active" action switches, re-evaluating sync status against the new plan's amounts. `linkedScheduledTransferId` handling across the switch defined at spec time (transfers stay owned by the active plan). (SPEC-009)
65d. [ ] **Data-shape obligations.** Backup format → `rmoney-data-v7` (+ RELEASE.md table row, SPEC-016 section); Storage-tab coverage for the plans collection; `updatedAt` stamping + tombstones + merge-engine coverage for sync (SPEC-039 conventions). (SPEC-009/016/026/039)
