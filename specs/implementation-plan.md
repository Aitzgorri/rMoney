...
2b. [ ] **Crypto entry forms (UI)** — in `InvestingAccountDetail.jsx`. **(a) Buy — done (2b-i):** `CryptoBuyForm` + `CoinSearchPicker` (D8: `searchCryptoCoins` candidates, top pre-selected → `setCryptoCoin`), `wallet` field, fractional `Quantity`, reuses the stock `handleBuy` param shape (cash balance + cross-currency FX) with `assetClass:'crypto'`. "+ Buy crypto" action button. Lint/build clean (not yet click-tested — coin search needs live CoinGecko). **(b) Sell — done (2b-ii):** per-row "Sell" button on the crypto holdings table → `CryptoSellForm` (coin fixed, quantity ≤ held with guard, price, fee), reuses `handleSell` with `assetClass:'crypto'` (FIFO crypto lots; proceeds to the trade-currency balance). Cross-currency proceeds deferred. **(c) Swap, (d) wallet Transfer — pending:** also from the holdings rows; format swap/wallet-transfer rows in the account history.
