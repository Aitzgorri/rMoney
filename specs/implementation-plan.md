...
8a. [ ] **Crypto valuation path + `cryptoProfiles` store** — new `rmoney_crypto_profiles` (`{ticker, coinId, name, confirmedAt}`) with CRUD + a **Settings→Storage card** (SPEC-026, mandatory for a new collection); `getCryptoPrice(ticker, vsCurrency, coinId)` in `marketDataClient` calling CoinGecko **directly** (hot-cached; `vsCurrency` = the holding's trade currency so existing `convertToMain` works). Foundation for 2b + 8b/8c.
