...
8b. [ ] **Crypto holdings view (D7)** — a dedicated, simple per-coin view (coin, wallet, quantity, current value, P/L) for crypto positions via `getPositions(acc,'crypto')` × `getCryptoPrice` × FX→main. Stock Inventory stays stock-only.
