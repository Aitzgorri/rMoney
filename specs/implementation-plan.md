...
2b. [x] **Crypto entry forms (UI)** — in `InvestingAccountDetail.jsx`. **(a) Buy (2b-i):** `CryptoBuyForm` + `CoinSearchPicker` (D8: candidates, top pre-selected → `setCryptoCoin`), `wallet` field, fractional `Quantity`, reuses `handleBuy` (cash balance + cross-currency FX) with `assetClass:'crypto'`. **(b) Sell (2b-ii):** per-row "Sell" → `CryptoSellForm`, reuses `handleSell` (FIFO crypto lots, proceeds to trade-currency balance). **(c) Swap (2b-iii, refined 2026-06-08):** per-row "Swap" → `CryptoSwapForm` (FROM held coin fixed, TO via coin picker, quantities + optional fee). **Auto-fetches both coins' live prices** (editable), derives `spotValue = fromQty × fromPrice`, and shows the implied vs market exchange rate + swap P/L; stores `from.price`/`to.price` on the record → `createSwap`. Coin candidate list made collapsible (`CoinSearchPicker`). **(d) wallet Transfer (2b-iv):** per-row "Move" → `CryptoWalletTransferForm` (quantity, from/to wallet) → `createWalletTransfer`. All build clean. **Deferred:** cross-currency sell proceeds; *editing* crypto buy/sell/swap via the edit dialogs (existing dialogs are stock-specific); not yet click-tested (coin search needs live CoinGecko).
...
6. [x] **CoinGecko adapter (D5)** — adapter module `services/providers/coingecko.js` built: keyless `getLatestPrice` + `getHistoricalSeries` priced in `config.vsCurrency`, symbol→id via highest-market-cap `resolveMarket` (session-cached), other methods throw. `api.coingecko.com` added to static CSP `connect-src` (SPEC-031); keyless ⇒ no secret/redaction/flag; no URL/key logging. Lint + build clean. *(Not yet imported — client registration + crypto-specific call path is item 8; disambiguation refinement is item 7.)*
7. [x] **Crypto ticker→coin resolution (SPEC-029)** — resolution primitive done: `searchCryptoCoins(query)` (client) → `coingecko.searchCoins` returns ranked `{coinId, symbol, name, marketCapRank}` candidates; the adapter's `getLatestPrice`/`getHistoricalSeries` accept `config.coinId` to price the chosen coin exactly (skips the symbol guess). Crypto-only: bypasses the stock chain. Stablecoins resolve + price like any coin, no peg map. Lint + build clean. *(User-facing pick + persisting the chosen coinId on the holding, plus its Storage-tab card, land with the entry form — item 2b.)*
8a. [x] **Crypto valuation path + `cryptoProfiles` store** — done: `data/cryptoProfiles.js` (`rmoney_crypto_profiles`, keyed by symbol; `getCryptoProfile`/`getCoinId`/`upsert`/`setCryptoCoin`/`delete` + storage-bytes helpers) with a **Settings→Storage card** ("Crypto coin mappings", SPEC-026). `getCryptoPrice(ticker, vsCurrency, coinId)` in `marketDataClient` calls CoinGecko directly, hot-cached under `crypto:<vs>` (no stock collision), `vsCurrency` = holding's trade currency. New code lints clean; build passes. *(Backup inclusion of the new collection = item 9.)*
8b. [x] **Crypto holdings view (D7)** — a dedicated "Crypto holdings" section in `InvestingAccountDetail` (alongside the stock positions table, where crypto is entered): per-coin rows (coin, quantity, avg cost, latest price, value trading/main, P/L main + %) via `getPositions(acc,'crypto')` enriched async with `getCryptoPrice(ticker, tradeCurrency, coinId)` × `convertToMain`, mirroring the stock enrichment. Read-only for now — Sell/Swap/Transfer buttons come with their forms (2b-rest). Wallet omitted from the aggregate table (coarse model, D3 — wallet lives in the transaction history). Stock Inventory unchanged. Lint/build clean (not click-tested — needs live CoinGecko).
8c. [x] **Investment Reports rollup (D7)** — done: `gatherRawRows` includes crypto positions (tagged `assetClass`, keyed `assetClass:ticker` to avoid symbol collisions); the price-fetch effect uses `getCryptoPrice` for crypto; `computeRows` values crypto (no dividends/HQ, name from `cryptoProfiles`); crypto contributes to the report total + weights. The **Crypto** investment-type chip is now live, `displayRows` filters by each row's class, and a new **By asset class** breakdown shows Stocks vs Crypto. Build passes; no new lint errors. *(Deferred: crypto in the portfolio-value-over-time chart.)*
9. [x] **Backup-format bump v4 → v5 (SPEC-016)** — done: `portability.js` adds `rmoney_crypto_profiles` to `KEYS`/export/import, `VERSION='rmoney-data-v5'`, and `v5` in `ACCEPTED_VERSIONS`; crypto transaction shapes ride inside the already-exported `rmoney_stock_transactions`. Additive (v1–v4 load transparently: missing `cryptoProfiles → []`, no-`assetClass` ⇒ stock). RELEASE.md *Data compatibility* table updated. Build passes.
