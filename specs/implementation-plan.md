...
| 66 — One-time income allocation rows | ✓ done (unreleased) | Shipped 2026-07-10 (174/174 tests): ⤵ per-envelope allocation rows linked to one one-time income (P2), distributed/overspend indicator, Apply → dated one-time transfers with sync/reset; 66d lifecycle follow-up — delete keeps recorded transfers by default (opt-in full undo) + per-income auto-fade once fully applied; SPEC-009 |
...
## Phase 66 — One-time income allocation rows ✓ done (2026-07-10, unreleased)

66a. ✓ **DONE** — **Allocation row model (4 new test suites' worth of pure helpers).** An allocation row is a planned-expense record with additive `allocationIncomeId` + `date` + `linkedEnvelopeTransferId` (no backup bump — v7 stays current; rides in the already-synced/carded collection). Created only manually via a **⤵ button on expense leaf rows** (hidden when the viewed plan has no one-time income); the form picks the income per row (auto when only one — P2), defaults the date to the income's date, fixes source = income's landing envelope and target = the row's envelope, and shows a live "already allocated · left" hint. Pure helpers in `data/planning.js`: `incomeAllocationSummary`, `plannedAllocationTransferFields`, `allocationSyncStatus`, `isAllocationRow` — unit-tested (summary/overspend, field mapping + rounding, full-field sync matrix, persistence). (SPEC-009)
66b. ✓ **DONE** — **Income-row distribution indicator.** One-time income rows show "distributed X of Y · Z left", flipping to a red "overspent by N" when allocations exceed the income (cent tolerance). (SPEC-009)
66c. ✓ **DONE** — **Apply → one-time transfers + full lifecycle.** Apply creates/updates a **one-time envelope transfer** dated as planned (never a rule); the apply dialog labels allocation items and notes the immediate dated recording. Allocation rows join the ●/↺ sync/reset machinery (compared against their one-time transfer; reset re-adopts its amount+date), render with a ⤵ date tag + MON-column-only amount, and are excluded from parent sums/% base while counting flat in the plan-balance totals (mirroring one-time incomes). Draft gating applies throughout. (SPEC-009)
66d. ✓ **DONE** (same-day follow-up — the user found applied transfers were destroyed on income delete, leaving no cleanup path) — **One-time income lifecycle.** Two-option delete dialog: **"Remove from plan"** (default — income + all allocation rows go, **created transfers stay recorded**; `deleteOneTimeIncomeCascade`) vs **"Also delete N transfers"** (explicit full undo, active plan only — a draft's links may be shared copies). Single allocation-row deletes now always keep their created transfer (history ≠ plan). New per-income **auto-fade** option on one-time incomes (`autoFade`, additive — no bump): the income + rows self-delete (transfers kept) once every allocation is applied in sync — `fadeSettledOneTimeIncomes(transfers, planId)` runs on Planning load and after Apply (active plan only; rowless incomes never fade); the income form offers manual (default) vs automatic with an explanatory hint, and the distribution indicator shows "fades when applied". 5 new tests (cascade keep/delete matrix, autoFade persistence, fade only-when-all-applied, manual/rowless never fade) — 174/174 green. (SPEC-009)
66e. ✓ **DONE** (same-day follow-up, user-reported) — **Allocation form: pickable target envelope + envelope-first default name.** The form had NO target-envelope picker — the target was locked to the clicked row's envelope, so a deep envelope without an expense row of its own was unreachable (the allocation silently landed on whichever envelope the nearest ⤵ row was bound to — the reported "assigns it to the parent"). Now an **"Into envelope" hierarchical dropdown** (indented tree, mandatory convention) defaults to the clicked row's envelope and is freely changeable; the default name became **"{target envelope} — from {income}"** (placeholder + empty-name fallback, live-updating). UI-only — the data layer stores whatever envelopeId it is given (already unit-tested). (SPEC-009)
66f. ✓ **DONE** (same-day follow-up, user-reported — root cause found via a Playwright reproduction against the live dev app) — **Active envelopes under an archived ancestor vanished from EVERY dropdown (app-wide latent bug).** `getEnvelopesFlat` walks strictly by `parentId`, so when callers pass `getActiveEnvelopes()`, filtering out an archived ancestor silently dropped its whole (still active) subtree — the reported "cannot assign an envelope with two parents / wrong default in the allocation form" was this: the deep envelope simply wasn't in the options. Fix: subtrees whose parent is missing from the passed set are appended as roots (envelopes AND the identical flaw in `getCategoriesFlat`). Regression tests written first (3 red → green, `flatDropdowns.test.js`); end-to-end re-verified with Playwright (archived middle parent → the ⤵ dialog now defaults to the clicked row's deep envelope). 178/178 tests. (SPEC-004/003)

## Phase 67 — Sharable-export Device-Sync scrub ✓ done (2026-08-21, unreleased)

67a. ✓ **DONE** — **Split the export redaction into two paths (resolves the 2026-07-10 SPEC-039 open question).** One `redactExportData` served two consumers with opposite needs: the device-sync payload *wants* `settings.sync` (that is how a second device learns the folder config) and *needs* `deletions` (`mergeSnapshots` reads the tombstone log from both sides so deleted records cannot resurrect), while a file handed to someone else must carry neither. The sync requirement won, so every Sharable `.rmy` leaked the user's WebDAV folder URL + username — mitigated only procedurally by RELEASE.md Step 5b. Now `data/portability.js` keeps `redactExportData` (credentials only — `utils/sync.js` `buildLocalSnapshot` still uses it, unchanged) and adds **`redactForFileExport`** = the same plus `delete out.deletions` and `delete out.settings.sync`; `App.jsx` `writeBackupFile` is its only caller. **No backup-format bump** — both modes still write `rmoney-data-v7`, the Sharable payload is a subset, and the loader already defaults `deletions ?? []` / falls back to the unconfigured `getSyncConfig()` shape. Full backups are untouched, which sharpens the two modes into *Full = rebuild your own device, Sharable = safe to hand over*; the trade-off (a Sharable restore starts with no tombstones, so a later first merge can resurrect records deleted elsewhere) is documented in SPEC-016, SPEC-039 and the README. New `data/portability.test.js` — 9 cases pinning the split from **both** directions, including the regression guard that the sync path still keeps `settings.sync` and `deletions`, since folding the strip into `redactExportData` would break deletion propagation with nothing in the UI to reveal it. 187/187 tests. (SPEC-039/SPEC-016)
