...
**Current phase: Phase 32 complete** *(All MVP feature phases complete: 3, 4b, 5b, 5c, 6, 6b, 7; Phases 8–18 and 22–31 mostly complete — 3 deferred items in Phase 26 remain; Phase 31 done — Dividend page (SPEC-032) complete; Phase 32 done — all sub-phases 32a–32k complete; one polish item remains deferred: item 382 standalone sell-row lot-picker button on planned rows, non-blocking)*
...
**Sub-phase 32f — Trading fees configuration (foundation for SPEC-034) ✓ DONE** *(items 371 + 372 complete; Settings → Investments tab gained a "Trading fees" card with two sections — per-exchange defaults pick from a new `CANONICAL_EXCHANGES` list in `marketDataExchanges.js` (MIC + name + default currency), and per-stock overrides pick from `getActiveStockProfiles()`. Both sections share an inline editor with Currency / Fee % / Minimum fee fields, duplicate-rule rejection, and a live `resolveTradingFee()` preview against a 1,000-unit sample. `getTradingFees() / setTradingFees() / resolveTradingFee(ticker, exchange, gross)` exported from `data/settings.js`; resolution order per-stock → per-exchange → none; computed as `max(minimumFee, gross × feePercent / 100)`. `feePercent` stored as the displayed percent value — e.g. `0.10` means 0.10 % of the trade — to avoid silent ×100 mistakes in the UI. Fees live inside the existing `rmoney_settings` blob, so no new Storage tab card is required.)*
...
**Sub-phase 32g — Buy-Sell Planning screen scaffold ✓ DONE**
373. [x] Add **Buy-Sell Planning** to the Investments nav second-row tab list (alongside `Investments overview / Portfolios / Watchlists / Benchmarks / Dividends`); create page route + screen file; empty-state "No scenarios yet — Create one to start planning" — entry added to `TopNav.jsx` + `BottomNav.jsx` (icon ⚖️); route `planning-trades` in `App.jsx` resolves to new screen `BuySellPlanning.jsx`
374. [x] `tradingScenarios` collection in localStorage; CRUD: create, rename, duplicate, delete; auto-save on every edit; scenario picker dropdown in page header showing the active scenario name — `data/tradingScenarios.js` exposes `getTradingScenarios / createTradingScenario / renameTradingScenario / duplicateTradingScenario / deleteTradingScenario`; the active id is held in a separate `rmoney_trading_scenarios_active` key so it persists across reloads. Auto-save is implicit: every mutator (`updateTradingScenario`, row helpers) writes the whole collection back to localStorage and stamps `updatedAt`
375. [x] Scenario stores: `{ name, sellRows, buyRows, cashTopUps, fxOverrides, displayedCurrencies, removeExecutedRows }` per the SPEC-034 Data block; row-add / row-remove operations work but compute calculations come in 32h — `blankScenario` matches the SPEC-034 Data block; row helpers `addSellRow / addBuyRow / updateSellRow / updateBuyRow / removeSellRow / removeBuyRow` are wired
375a. [x] Add-row controls: row-add for buys opens a stock-picker modal (existing or SPEC-029-resolved tickers); row-add for sells opens a position-picker scoped to currently-held positions across all investing accounts — `StockPicker` reads `getActiveStockProfiles()`; `PositionPicker` groups `getOpenLots()` results by ticker so each ticker appears once with its largest-holding account selected by default
375b. [x] Sell row defaults: investing-account picker shows only accounts that hold the stock; defaults to the largest position when multiple accounts qualify — `accountsHolding(ticker)` filters per the open-lots map; PositionPicker picks the largest account on add; the row-level account dropdown is restricted to holding accounts and re-clamps the share count when switching account
375c. [x] Buy row defaults: investing-account picker offers all investing accounts; defaults to the most-recently-used account — `getLastBuyAccountId()` reads `rmoney_trading_scenarios_last_buy_account`; `addBuyRow` updates this key on every buy add or buy-row account change. Falls back to `accounts[0]` when the stored id is no longer valid
375d. [x] Sell row Number-of-shares input is constrained to ≤ available shares on the picked account (matches the SPEC-019 per-lot bound from item 367 applied here at row level) — `handleUpdateSellShares` clamps at write time; the input also carries `max={availableShares}` and a "max N" hint inline next to the field
375e. [x] Portability — new `tradingScenarios` collection registered in both Sharable and Full backup paths in `data/portability.js`

**Sub-phase 32h — Buy-Sell Planning calculations + storage card ✓ DONE** *(Execute delivered in 32k)*
376. [x] Sell rows + Buy rows render via the shared `ConfigurableTable` with the column sets defined in SPEC-034. Always-visible columns are non-removable; toggleable columns persist visibility per-table in localStorage — `buildSellColumns` + `buildBuyColumns` in [BuySellPlanning.jsx](app/src/screens/BuySellPlanning.jsx); persistence key `rmoney_bsp_{side}_columns_{scenarioId}`
376a. [x] Sell-table column set — implemented per SPEC-034: include / ticker / account / shares-to-sell / available-shares (with `(N LT)` subline) always visible; name / exchange / currency / FX-rate-vs-main / last price / adjusted price / fee amount / fee % / fwd-yield / fwd-month-gross-net / TTM-yield / TTM-month-gross-net / trade-gross / net-of-fee / main-currency value toggleable; default-hidden the verbose monthly/main columns to keep the first render readable
376b. [x] Buy-table column set — implemented per SPEC-034: include / ticker / account / shares-to-buy always visible; name / exchange / currency / FX-rate-vs-main / last price / adjusted price / fee amount / fee % / buy-price-including-fee-per-share / fwd-yield / fwd-month-gross-net / TTM-yield / TTM-month-gross-net / trade w/o fee / trade w/ fee / main-currency value toggleable
377. [x] Adjusted-price control per row: `Last price` / `Round down to N decimals` / `Round up to N decimals` / `Manual`; drives every downstream row calculation. Decimal-count input shown when round rule selected — `AdjustedPriceCell` widget; `applyAdjustedPriceRule` in [planningCalc.js](app/src/utils/planningCalc.js)
378. [x] Fee handling per row: pre-fills from `resolveTradingFee()` (Sub-phase 32f); inline override per row sets `manualFeeOverride` on the row; small dot indicator marks overridden rows; tooltip on Fee cell points to Settings → Investments → Trading fees — `FeeCell` widget; per-row override revert button (↺) clears `manualFeeOverride`
378a. [x] Dividend % columns use the per-row `(adjustedPrice + fee/share)` as the **denominator** for buys (sells use bare `adjustedPrice`) so the fee impact appears in the planned-trade yield; per-month gross = numerator × shares ÷ 12; per-month net = gross × (1 − resolved tax %) — `computeDividendColumns` in [planningCalc.js](app/src/utils/planningCalc.js)
379. [x] Overview block: cash balances panel (per-currency totals across all investing accounts), per-currency planning-only top-up inputs, FX-rates panel (live default + user override), cash-impact table (Start / Top-up / Sells / Buys / Transfer in / Transfer out / End per displayed currency), weighted-average dividend metrics rows (sells / buys / delta in gross + net) — `OverviewBlock` in [BuySellPlanning.jsx](app/src/screens/BuySellPlanning.jsx)
379a. [x] Currency-display picker in overview: user ticks which currencies the summary totals are expressed in; defaults to all trade currencies of included rows + main currency
379b. [x] FX rates panel auto-populates the pairs the calculation actually uses (trade-currency → main-currency for every distinct trade currency); pre-fills with live SPEC-027 rates from `getCachedRates()`; user can override; overrides apply only to this scenario's calc — overrides stored in `scenario.fxOverrides` keyed by `'FROM->TO'`
380. [x] Currency-exchange priority for cash-impact: matching trade currency → main currency → other balances by descending available value. Implemented in pure helper `simulateCashImpact(...)` in [planningCalc.js](app/src/utils/planningCalc.js#L132) so it is testable in isolation; reports `shortfall` per currency when buys exceed available cash even after the cascade
381. [x] Sell row "Available shares" displays `(N held > 365 days)` long-term-hold count from `getOpenLots(..., asOfDate=today)` filtered by buy-date age. Subline tooltip explains the count is informational — `longTermSharesCount()` in planningCalc; `(N LT)` rendered under the Available cell
382. [ ] Standalone sell-row lot-picker button (stores lot selections back to the planned row without executing) — **still deferred**; the lot picker IS available inside the Execute modal (see 32k). A standalone lot-picker-only button is a polish item.
383. [x] **Execute** action button per row opens `ExecuteModal` (self-contained in `BuySellPlanning.jsx`) pre-filled with account (locked), ticker, shares, adjusted price, fee, lot picker for sells (FIFO). Calls `createBuy` / `createSell`, then `markRowExecuted`. Row shows "✓ Done" badge and Include checkbox is forced off. Cancel leaves the row untouched. *(Sub-phase 32k)*
384. [x] Per-scenario "Remove executed rows on save" toggle in scenario actions menu (default off — keep history) — toggle lives inline in the scenario action bar; flag stored as `scenario.removeExecutedRows`; consumed by `markRowExecuted`
385. [x] Settings → Storage tab: "Buy-Sell Planning" card added with per-scenario breakdown (sell-row count, buy-row count, bytes), total summary, and bulk-clear action — uses the same pattern as the Manual stock prices card

**Sub-phase 32k — Execute action + lot-picker reuse ✓ DONE** *(item 383 complete; `ExecuteModal` self-contained in `BuySellPlanning.jsx` — no changes to `InvestingAccountDetail.jsx`; lot picker inside the modal uses the same FIFO logic + per-lot clamping as `SellForm`; balance preview shows current→projected balance per currency; calls `createBuy`/`createSell` + `markRowExecuted`; ▶ Execute button disabled until price is known; ✓ Done badge replaces button on executed rows; standalone lot-picker-only button on planned sell rows remains deferred as item 382)*

**Sub-phase 32i — Generic edit/delete control discoverability audit (cross-spec) ✓ DONE** *(item 386 complete; removed `opacity: 0` / hover-reveal pattern from five screens — `Investments.module.css` (`.cardActions`), `InvestingAccountDetail.module.css` (`.movementDeleteBtn`), `Envelopes.module.css` (`.actions`), `Categories.module.css` (`.actions`), `Portfolios.module.css` (`.nodeActions` + `.assignActions`); buttons now always visible; `movementDeleteBtn` colour raised from `#3d4558` to `#64748b` for contrast; `aria-label` added to every icon-only button across `Investments.jsx`, `InvestingAccountDetail.jsx`, `Envelopes.jsx`, `Categories.jsx`, `Portfolios.jsx`; drag-handle opacity-0 patterns left intentional — they are affordances, not controls; Accounts + Transactions use full-row-click-to-edit, no separate icon buttons — those patterns are intentional and need no change)*
