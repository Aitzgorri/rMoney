...
2b. [ ] **Crypto entry forms (UI)** — in `InvestingAccountDetail.jsx`: (a) crypto **buy/sell** mode — `wallet` input in the `exchange` slot, fractional-quantity input, and the **coin picker** (D8: `searchCryptoCoins` candidates with the top pre-selected) that persists the chosen `coinId` to `cryptoProfiles`; calls `createBuy`/`createSell` with `assetClass:'crypto'`, `wallet`, `coinId`. (b) **Swap** form → `createSwap`. (c) **wallet Transfer** form → `createWalletTransfer`; format swap/wallet-transfer rows in the account history. Depends on item 8a.
