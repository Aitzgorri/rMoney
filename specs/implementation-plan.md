...
53a. [ ] **Account prefill on the mobile add-transaction route.** `AddTransaction.jsx` doesn't pass `defaultAccountId`, so the mobile form always defaults to the first account while the desktop inline form prefills filter → last-used (Phase 51d). **Clarified with the user 2026-07-08:** "selected account" = the account filtered via the account buttons on the Transactions screen (which exist on mobile too, `Transactions.jsx:281-290`). The filter lives in `Transactions.jsx` component state (seeded from `initialAccountId`), while the ＋-menu Add-transaction is a separate route — so lift the active account filter to App-level state (or a small persisted UI pref) and pass it to `AddTransaction` → `TransactionForm` as `defaultAccountId`, falling back to last-used when no filter is active. Same rule as desktop: filter → last-used. (SPEC-005/006)
53b. [ ] **Planning-screen frequency list → shared module.** `Planning.jsx:859-864` still hard-codes `one-time / monthly / quarterly / yearly` (day-of-month only) instead of consuming `utils/frequency.js` — the one recurrence dropdown left outside the Phase 47 unification. **Scope decision needed at build time:** offer the full shared set (adds weekly/bi-weekly to planned incomes, needs weekday-picker support in the Planning form + period-attribution math) or keep the subset but source it from `FREQUENCIES` so labels/order stay aligned. (SPEC-009)
53c. [ ] **Scheduled-transfers page: human-readable Freq/Day columns.** `ScheduledTransfers.jsx` renders raw stored values (`biweekly`, weekday integer `2`) in its desktop columns; route them through `FREQUENCY_LABELS` / `dayLabel` like the envelope detail pane does. (SPEC-012)
53d. [ ] **Residual UTC "today" defaults.** `EnvelopeTransferForm.jsx:30` and `data/envelopes.js:276` still default dates via `new Date().toISOString()` — near midnight in a UTC+ timezone the default date is yesterday. Switch both to `localDateStr` (`utils/dates.js`). Same bug family as the fixed 16→15 issue. (SPEC-004/012)
53e. [ ] **Favorites + payee→category memory beyond the transaction form.** Wire the Phase 48 favorites into the remaining entity dropdowns — Bills & Income `PlannedItemForm` (account/category/envelope) and `EnvelopeTransferForm` (from/to envelope) — reusing `splitFavorites` + the Phase 51c rendering conventions (★ + divider for flat lists, `Favorites` optgroup above the indented tree). Add payee→category prefill (`getRecentCategoriesForPayee`) to the Bills & Income form on payee entry. (SPEC-012/013)
53f. [ ] **(Enhancement) Optional start date on scheduled transfers.** Bi-weekly/quarterly/yearly scheduled transfers anchor on `createdAt` today, so the user cannot control which fortnight / which month of the quarter fires. Add an optional `startDate` (default = created date, so existing records keep behaviour); `isScheduledTransferDueToday` + `nextScheduledOccurrence` anchor on it when present. Additive field inside the existing `rmoney_envelope_scheduled` blob — no backup-format bump; also enables "starts next month" setups. (SPEC-012)
53g. [ ] **(Enhancement) Payee→envelope memory in the transaction form.** Mirror the Phase 51f category memory for envelopes: on payee entry with no envelope chosen, prefill the last envelope used for that payee, and show the last 3 distinct envelopes in a "Recent for this payee" optgroup above Favorites. Same derivation from transaction history, no new storage. (SPEC-005)
...
54a. [ ] **Planning-apply rounding (+ repair migration).** Root cause of the misbehaving balance in the screenshot: `applyExpense` (`Planning.jsx:360-378`) persists raw `convertAmount` output (yearly ÷ 12 → e.g. 51.66/12 = **4.305**) into transfer + scheduled-transfer amounts; sums then disagree with their per-row rounded display (header 0,01 vs running 0,00). Fix: `round2` the monthly amount in `applyExpense`; coerce-round to 2dp inside `createEnvelopeTransfer` / `createScheduledTransfer` / `updateScheduledTransfer` so **no** path can persist >2dp; startup repair migration for existing stored amounts (Phase 43a–c pattern). No data-shape change, no backup bump. (SPEC-009/004)
54b. [ ] **Transfer From-account prefill.** `TransactionForm.jsx:48` hard-codes `sourceAccountId: accounts[0]` — extend the Phase 51d `defaultAccountId` (filter → last-used) to the transfer branch's From account. (SPEC-005)
54c. [ ] **Envelope detail-pane polish.** (1) `title` tooltips on the ⚙ filter and ↓/↑ sort buttons (`EnvelopeHistory.jsx:239-243`); (2) the detail-header ⇄ gains a "Transfer" text label (the left-pane button already has one); (3) the left tree-pane "⇄ Transfer" button passes the selected envelope as `defaultFromEnvelopeId` (today it always falls back to Undistributed income — inconsistent with the detail-header button, which does prefill); (4) new action button between Transfer and filter toggling a per-day "spendable until period end" figure in the Balance row, reusing the Dashboard widget formula (`getTotalEnvelopeBalance ÷ daysRemaining()`, `Dashboard.jsx:417-419`). (SPEC-007)
54d. [ ] **App-wide tooltip audit.** One-time sweep adding `title` to every button missing one, per the new MANDATORY CLAUDE.md rule (D2). (SPEC-015)

## Phase 55 — Bills & Income editing + confirmation overhaul (planned)

55a. [ ] **Edit scope: "from now on" default + opt-in past rewrite (fixes the edit→transaction bug).** Today `handleSaveItem` (`BillsAndIncome.jsx:74-82`) unconditionally runs `checkAndGeneratePending()`, so a schedule-affecting edit of an **auto-apply** item backfills transactions for new due dates ≤ today on the spot. Fix per D1: an edit re-anchors generation to the future (no backfill; a transaction on save only if the newly chosen recurrence day **is** today), and the form shows "next occurrence: {date}" feedback. Optional per-edit "also change past records": previews "N records since {first occurrence}" (N = the item's confirmed occurrences' linked transactions), then rewrites those transactions — **amount only** (locked with the user 2026-07-08): the scope option's label/description must state that only the amounts of past records change, and that their dates, accounts, categories, envelopes and payees are not touched. (SPEC-013)
55b. [ ] **Planning Apply-next must not record today.** The ApplyDialog `next` scope (`Planning.jsx:375`) creates an envelope transfer dated **today** regardless of the recurrence day — apply the same rule as 55a: take effect at the next occurrence (shown in the dialog), transfer created only when due. (SPEC-009)
55c. [ ] **Dashboard upcoming: show due-pending + confirm inline.** `getUpcomingOccurrences` excludes items with a pending occurrence (`bills.js:240-249`), so exactly the items awaiting confirmation are invisible on the Dashboard, and the card is read-only. Add due-pending rows (distinct styling) with a Confirm action reusing `confirmOccurrence`, same semantics as the Bills & Income page. (SPEC-008/013)
